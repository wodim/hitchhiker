Func globalf()
    ...
    ElseIf $wowprocesslistviewitemcount[0][0] = $wowprocesslistviewitemcountsave Then
        ...
        If GUICtrlRead($privecollisionsall) = $gui_checked Then
            ...
            _memorywrite($collisionwmo, $wowprocess, -1070428016, "Ptr")
        EndIf
        If GUICtrlRead($privewallclimbon) = $gui_checked Then
            _memorywrite($playerbase + $wallclimb, $wowprocess, 255, "float")
        EndIf
        If GUICtrlRead($priveunderwaterwalkon) = $gui_checked AND GUICtrlRead($privewalljumpon) = $gui_checked Then
            _memorywrite($playerbase2 + $playerflags, $wowprocess, 4, "dword")
        ElseIf GUICtrlRead($priveunderwaterwalkon) = $gui_checked Then
            _memorywrite($playerbase2 + $playerflags, $wowprocess, 6, "dword")
        ElseIf GUICtrlRead($privewalljumpon) = $gui_checked Then
        ...
        If _memoryread($playerbase2 + $ressources, $wowprocess, "dword") = 0 Then
            If GUICtrlRead($privetrackherbes) = $gui_checked OR GUICtrlRead($privetrackfilons) = $gui_checked OR GUICtrlRead($privetrackcoffres) = $gui_checked OR GUICtrlRead($privetrackpoissons) = $gui_checked Then
        ...
        If _memoryread($playerbase2 + $hunt, $wowprocess, "dword") = 0 Then
            ...
            EndIf
        EndIf
        If _guictrlcombobox_getcursel($privefactionmenu) = 1 Then
            _memorywrite(($playerbase2 + $faction), $wowprocess, 0, "dword")
        ElseIf _guictrlcombobox_getcursel($privefactionmenu) = 2 Then
            _memorywrite(($playerbase2 + $faction), $wowprocess, 1, "dword")
        ElseIf _guictrlcombobox_getcursel($privefactionmenu) = 3 Then
